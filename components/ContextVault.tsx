...
export function ContextVault({ projectId, blocks }: Props) {
  ...
  return (
    <section className="space-y-3">
      ...
      {adding && (
        <div className="border border-zinc-200 rounded-md bg-white p-3 space-y-2">
          <input
      ...
      <ul className="space-y-2">
        ...
        {blocks.map((b) => (
          ...
          >
            ...
            {b.tags.length > 0 && (
              <div className="flex flex-wrap gap-1 mt-1">
                {b.tags.map((t) => (
                  <span
                    key={t}
                    className="text-[10px] uppercase tracking-wide text-zinc-500 bg-zinc-100 rounded px-1.5 py-0.5"
                  >
                    {t}
                  </span>
                ))}
            ...
            <pre className="mt-2 text-xs text-zinc-700 whitespace-pre-wrap font-sans line-clamp-4">
              {b.content}
            </pre>
          </li>
        ))}
